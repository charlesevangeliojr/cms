...
<aside id="adminSidebar" class="fixed inset-y-0 left-0 z-40 flex h-screen w-64 -translate-x-full shrink-0 transform flex-col bg-gray-900 text-gray-200 transition-transform duration-200 ease-in-out md:sticky md:top-0 md:translate-x-0">
    <div class="flex shrink-0 items-start justify-between gap-3 border-b border-gray-800 px-6 py-5">
        ...
        <button type="button" onclick="toggleAdminSidebar(false)" aria-label="Close navigation menu" class="rounded-lg p-1.5 text-gray-400 hover:bg-gray-800 hover:text-white md:hidden">
    ...
    <nav aria-label="Admin navigation" class="min-h-0 flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        <a href="{{ route('contacts.index') }}" aria-current="{{ request()->routeIs('contacts.*') ? 'page' : 'false' }}"
           class="flex items-center gap-3 px-3 py-2 rounded text-sm font-medium {{ request()->routeIs('contacts.*') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
        ...
        <a href="{{ route('newsletters.index') }}" aria-current="{{ request()->routeIs('newsletters.*') ? 'page' : 'false' }}"
           class="flex items-center gap-3 px-3 py-2 rounded text-sm font-medium {{ request()->routeIs('newsletters.*') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
        ...
        <a href="{{ route('banners.index') }}" aria-current="{{ request()->routeIs('banners.*') ? 'page' : 'false' }}"
           class="flex items-center gap-3 px-3 py-2 rounded text-sm font-medium {{ request()->routeIs('banners.*') ? 'bg-gray-800 text-white' : 'text-gray-300 hover:bg-gray-800 hover:text-white' }}">
    ...
    <div class="shrink-0 border-t border-gray-800 px-3 py-4">
